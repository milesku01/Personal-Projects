...
public class NetworkEvaluator {
	...
	public void predict(String model, double ...inputs) {
		...
		fr = new FileReader(filePath + model + ".txt");
		...
		mean = new double[numofLayers];
		strdDev = new double[numofLayers];
	...
	public void forwardPropagation() {
		...
		for (int i = 1; i < numofLayers - 1; i++) {
			...
			System.out.println("Layer" + java.util.Arrays.deepToString(layerValue));
